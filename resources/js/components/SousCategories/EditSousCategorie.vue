...
<form @submit.prevent="modifierScategorie">
 ...
 <input type="text" class="form-control" placeholder="Nom" vmodel="SousCategories.nomscategorie">
 </div>
 <div class="form-group">
 <FilePond max-files="1" @change="onFileChange" :files="'../storage/images/'+SousCategories.imagescat"/>
</div>
 <div class="form-group">  Catégories<select class="form-control" vmodel="SousCategories.categorieID">

 <option v-for="c in Categories" :key="c.id" :value="c.id">{{c.nomcategorie}}</option>
 </select>
 </div>
 <button type="submit" class="btn btn-block btn-primary">Modifier Sous Catégorie</button>
</form>
</template>
<script>
 ...
 this.axios.get('http://localhost:8000/api/categories').then(res =>
{
 ...
 getOneScategorie(){
 this.axios
 .get(`http://localhost:8000/api/scategories/${this.$route.params.id}`)
 .then((res) => {
