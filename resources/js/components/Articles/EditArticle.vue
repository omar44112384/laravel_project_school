...
<form @submit.prevent="modifierproduit">
 ...
 <div class="form-group">
 <input type="number" class="form-control" placeholder="qtestock" vmodel="article.qtestock">
 </div>
 <div class="form-group">
...
export default {
 ...
 onFileChange(e) {
 this.article.imageartpetitf = '../storage/images/'+e.target.files[0].name;
 ...
 this.axios.get('http://localhost:8000/api/scategories/'+categ).then(res =>
{
 ...
 getOneArticle(){
 this.axios
 .get(`http://localhost:8000/api/articles/${this.$route.params.
id}`)
 .then((res) => {
